...
def report():
    ...
    if ip is None:
        # If the request is not proxied, consider the remote address is the client's address
    ...
    # ip, user_agent, cookies, request_data, timestamp, headers
    ...
    if not os.path.exists(report_file_path):
        with open(report_file_path, 'w') as f:
            ...
            f.write(f"{ip},{ua},{cookies},{request_data},{timestamp},{headers}\n")
    else:
        with open(report_file_path, 'a') as f:
            f.write(f"{ip},{ua},{cookies},{request_data},{timestamp},{headers}\n")
    print(f"{ip},{ua},{cookies},{request_data},{timestamp},{headers}\n")
    response = {
        "status": "OK"
    }
    return response
